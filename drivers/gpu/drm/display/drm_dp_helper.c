 ...
 *
 * Returns:
 * The maximum overall DSC pixel throughput supported by the branch device
 * in kPixels/sec or 0 if the branch device doesn't indicate the max overall
 * throughput capability separately and this must be determined by the
 * per-slice max throughput (see @drm_dp_dsc_branch_slice_max_throughput())
