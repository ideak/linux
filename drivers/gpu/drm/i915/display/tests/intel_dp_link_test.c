...
/*
 * TEST: Fixed reference table
 * ---------------------------
 * Verify the link_caps config iterator using fixed standard DP config tables.
 */
static void test_fixed_ordered_table(struct kunit *test,
...
static void intel_dp_link_caps_test_fixed(struct kunit *test)
{
	struct test_ctx *ctx = test->priv;
	...
	ctx->link_caps_ops->update(link_caps,
	...
	for (i = 0; i < ARRAY_SIZE(config_orders); i++)
		test_fixed_ordered_table(test, link_caps, config_orders[i]);
}
 ...
 * combinations.
 */
static u32 get_link_caps_enabled_config_mask(struct kunit *test, struct intel_dp_link_caps *link_caps)
{
	struct test_ctx *ctx = test->priv;
	 ...
	 */
	ctx->link_caps_ops->get_max_limits(link_caps, &old_limits);
	ctx->link_caps_ops->reset_max_limits(link_caps);

	mask = ctx->link_caps_ops->get_allowed_config_mask(link_caps);

	ctx->link_caps_ops->set_max_limits(link_caps, &old_limits);
			   ...
			   struct intel_dp_link_caps_config_order config_order)
{
	...
	test_for_each_dp_link_config_idx(ctx, config_order, get_all_config_mask(),
		...
		/*
		 * Update the max limits and max BW for allowed configs,
		 * verified at the end for the whole config table.
		 */
		if (!(ctx->link_caps_ops->get_allowed_config_mask(link_caps) &
		      BIT(iter_config_idx)))
			continue;
	...
	ctx->link_caps_ops->get_max_limits(link_caps, &actual_max_limits);
				       ...
				       const struct link_config_set *config_set)
{
	...
	for (i = 0; i < config_set->size; i++) {
		...
		config_idx = ctx->link_caps_ops->find_allowed_config_idx(link_caps,
		...
		ctx->link_caps_ops->disable_config(link_caps, config_idx);
...
static void test_update_rates_shrink(struct kunit *test, bool disable_configs)
{
	...
	while (expected_table.rates.size > 1) {
		...
		update_link_caps_and_verify(&expected_table, link_caps, INTEL_DP_LINK_CAPS_UPDATE_MERGE);
	}
}
...
static void intel_dp_link_test_fallback_for_edp(struct kunit *test)
{
	...
	test_for_each_dp_link_config(ctx,
				ctx->link_caps_ops->config_order_for_connector(&ctx->dev.connector),
				ctx->link_caps_ops->get_allowed_config_mask(link_caps),
				&iter_config)
		break;
	...
	fallback_err = ctx->link_training_ops->get_fallback_values(&ctx->dev.dp, &ctx->dev.crtc_state);
				      ...
				      const struct intel_dp_link_config *expected_fallback_config)
{
	...
	struct intel_dp_link_caps_config_order connector_desc_order = {
		.key = ctx->link_caps_ops->config_order_for_connector(&ctx->dev.connector).key,
		.dir = INTEL_DP_LINK_CAPS_CONFIG_ORDER_DIR_DESC,
	};
	...
	KUNIT_EXPECT_TRUE(test, link_configs_match(&iter_config, expected_target_config));
	KUNIT_EXPECT_FALSE(test, link_configs_match(&iter_config, &INTEL_DP_LINK_CONFIG_NULL));
	...
	if (!fallback_err) {
		 ...
		 * selection logic.
		 */
		add_config(test, &expected_table->disabled_configs, expected_target_config);
	}
...
static struct kunit_case intel_dp_link_test_cases[] = {
	KUNIT_CASE(intel_dp_link_caps_test_fixed),
