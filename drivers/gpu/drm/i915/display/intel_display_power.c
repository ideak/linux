...
#define for_each_power_domain_well(__dev_priv, __power_well, __domain)	\
	for_each_power_well(__dev_priv, __power_well)				\
		for_each_if(test_bit((__domain), (__power_well)->domains))
				      ...
				      struct i915_power_well *power_well)
{
	...
	bitmap_and(domain_mask, domain_mask, power_well->domains, POWER_DOMAIN_NUM);
	refs = bitmap_weight(domain_mask, POWER_DOMAIN_NUM);
...
assert_async_put_domain_masks_disjoint(struct i915_power_domains *power_domains)
{
	...
	return !drm_WARN_ON(&i915->drm,
			    bitmap_intersects(power_domains->async_put_domains[0],
					      power_domains->async_put_domains[1],
					      POWER_DOMAIN_NUM));
}
...
__async_put_domains_state_ok(struct i915_power_domains *power_domains)
{
	...
	err |= drm_WARN_ON(&i915->drm,
			   ...
			   !bitmap_empty(async_put_mask, POWER_DOMAIN_NUM));
				...
				const char *prefix, intel_power_domain_mask_t mask)
{
	...
	drm_dbg(&i915->drm, "%s (%d):\n", prefix, bitmap_weight(mask, POWER_DOMAIN_NUM));
			       ...
			       enum intel_display_power_domain domain)
{
	...
	clear_bit(domain, power_domains->async_put_domains[0]);
	clear_bit(domain, power_domains->async_put_domains[1]);
}
				       ...
				       enum intel_display_power_domain domain)
{
	...
	if (!test_bit(domain, async_put_mask))
		goto out_verify;
	...
	async_put_domains_mask(power_domains, async_put_mask);
	if (!bitmap_empty(async_put_mask, POWER_DOMAIN_NUM))
		goto out_verify;
				 ...
				 enum intel_display_power_domain domain)
{
	...
	async_put_domains_mask(power_domains, async_put_mask);
	drm_WARN(&dev_priv->drm,
		 test_bit(domain, async_put_mask),
...
intel_display_power_put_async_work(struct work_struct *work)
{
	...
	if (!bitmap_empty(power_domains->async_put_domains[1], POWER_DOMAIN_NUM)) {
		bitmap_copy(power_domains->async_put_domains[0],
			    power_domains->async_put_domains[1],
			    POWER_DOMAIN_NUM);
		bitmap_zero(power_domains->async_put_domains[1],
			    POWER_DOMAIN_NUM);
				     ...
				     intel_wakeref_t wakeref)
{
	...
	if (power_domains->async_put_wakeref) {
		set_bit(domain, power_domains->async_put_domains[1]);
	} else {
		set_bit(domain, power_domains->async_put_domains[0]);
		queue_async_put_domains_work(power_domains,
			       ...
			       enum intel_display_power_domain domain)
{
	...
	drm_WARN_ON(&i915->drm, test_bit(domain, power_domain_set->mask));

	wf = intel_display_power_get(i915, domain);
#if IS_ENABLED(CONFIG_DRM_I915_DEBUG_RUNTIME_PM)
	power_domain_set->wakerefs[domain] = wf;
#endif
	set_bit(domain, power_domain_set->mask);
}
				    ...
				    intel_power_domain_mask_t mask)
{
	...
	drm_WARN_ON(&i915->drm, !bitmap_subset(mask, power_domain_set->mask, POWER_DOMAIN_NUM));
	...
	for_each_power_domain(domain, mask) {
		...
		clear_bit(domain, power_domain_set->mask);
	}
}
